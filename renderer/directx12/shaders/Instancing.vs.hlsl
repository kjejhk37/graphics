// Author: Claude
// Description: DX12 Instancing Vertex Shader. 정점(POSITION)은 입력 슬롯 0에서 매 정점마다,
...
//        4개의 TEXCOORD 스타일 float4 원소로 분해되어 전달됨 - D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA)
// Output: SV_POSITION(변환된 클립 공간 좌표)
// Notes: 카메라/투영 시스템은 범위 밖이라 ViewProj는 Identity를 사용한다(DirectX12Renderer 참고).
